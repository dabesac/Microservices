...
module.exports = function(app) {
  ...
  findAllTshirts = function(req, res) {
    console.log("GET - /tshirts/size");
    return Tshirt.find({}).exec(function(err, tshirts) {
      if(!err) {
  ...
  findAllTshirtsSize = function(req, res) {
    console.log("GET - /tshirts/size");
    return Tshirt.find({}).sort('size').exec(function(err, tshirts) {
      if(!err) {
        return res.send(tshirts);
      } else {
        res.statusCode = 500;
  ...
  findAllTshirtsPriceAsc = function(req, res) {
    console.log("GET - /tshirts/priceAsc");
    return Tshirt.find({}).sort('price').exec(function(err, tshirts) {
      if(!err) {
        return res.send(tshirts);
      } else {
        res.statusCode = 500;
  ...
  findAllTshirtsPriceDesc = function(req, res) {
    console.log("GET - /tshirts/priceAsc");
    return Tshirt.find({}).sort('-price').exec(function(err, tshirts) {
      if(!err) {
        return res.send(tshirts);
  ...
  app.get('/catalogo/priceAsc', findAllTshirtsPriceAsc);
  app.get('/catalogo/priceDesc', findAllTshirtsPriceDesc);
  app.get('/catalogo/size', findAllTshirtsSize);
  app.get('/catalogo', findAllTshirts); 
